...
Signal::Exciter::Exciter(String name, CPS::Logger::Level logLevel) 
	...
	addAttribute<Real>("Vm", &mVm, Flags::read | Flags::write);
	addAttribute<Real>("Vr", &mVr, Flags::read | Flags::write);
	addAttribute<Real>("Vf", &mVf, Flags::read | Flags::write);
	addAttribute<Real>("Vis", &mVis, Flags::read | Flags::write);
	addAttribute<Real>("Vse", &mVf, Flags::read | Flags::write);
	addAttribute<Real>("Vf", &mVf, Flags::read | Flags::write);
}
...
Real Signal::Exciter::step(Real mVd, Real mVq, Real dt) {
	...
	// mVse = mVf * (0.33 * (exp(0.1 * abs(mVf)) - 1.));
	mVse = mVf * (0.33 * exp(0.1 * abs(mVf_prev)));
